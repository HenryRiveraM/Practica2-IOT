...
class ESPCRUDProtocol:
    ...
    def _process_command(self, cmd, res, dev_id, data):
        ...
        resource_type = resource_map.get(self.Resource(res), None)
        ...
        with self.lock:
            try:
                ...
                if cmd == self.Command.POST:
                    ...
                    header = struct.pack('!BBHL', self.Status.OK, res, dev_id, len(value))
                    return header + value
    ...
    def start(self):
        ...
        try:
            ...
            while self.running:
                ...
                thread = threading.Thread(target=self._client_handler, args=(conn, addr), daemon=True)
                thread.start()
...
if __name__ == "__main__":
    # 0.0.0.0 = escucha en todas las interfaces (perfecto para tu caso)
